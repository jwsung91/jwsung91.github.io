...
[설계 배경](/blog/2026-05-31-wirestead-설계-배경)에서는 wirestead를 만든 배경과 설계 방향을 정리했다.
...
wirestead에서는 이 문제를 줄이기 위해 `wirestead.hpp`를 public entry point로 두고, 그 아래에 Facade, Builder, Wrapper 구조를 배치했다.
...
이 글에서는 wirestead의 public API가 어떤 구조로 정리되어 있는지, 그리고 왜 Facade, Builder, Wrapper 중심으로 구성되었는지 살펴본다.
...
wirestead의 public API는 다음 역할을 담당한다.
...
## wirestead.hpp: Public Facade

wirestead의 public API 중심에는 `wirestead.hpp`가 있다.

`wirestead.hpp`는 단순히 여러 header를 모아둔 include 파일이 아니다.
...
사용자는 내부 namespace와 구현 파일을 세부적으로 찾아 들어가지 않고도, `wirestead.hpp`를 통해 주요 public API에 접근할 수 있다.
...
wirestead는 내부적으로 여러 wrapper, builder, transport, diagnostics, memory, framer 계층을 가진다. 다만 사용자가 처음 의존해야 하는 public surface를 하나의 진입점으로 정리했다는 의미에 가깝다.

즉, `wirestead.hpp`는 내부 구조를 없애는 것이 아니라, 내부 구조를 사용자가 직접 탐색하지 않아도 되게 만드는 Facade다.
...
wirestead의 Facade는 다음과 같은 기준으로 public API를 정리한다.
...
예를 들어 사용자는 `wrapper::TcpClient`라는 내부 namespace를 직접 사용할 수도 있지만, public API에서는 `wirestead::TcpClient` alias로 접근할 수 있다.
...
사용자 입장에서는 “어디에 구현되어 있는가”보다 “무엇을 사용할 수 있는가”가 중요하다. wirestead namespace에서 핵심 타입을 바로 노출하면 API 탐색 비용이 줄어든다.

## Builder Entry Point

wirestead의 객체 생성은 생성자를 직접 호출하는 방식보다 builder entry point를 통해 시작된다.
...
여기서 `wirestead::tcp_client(...)`는 곧바로 TCP client 객체를 만드는 함수가 아니라, TCP client builder를 반환하는 진입점이다.
...
```mermaid
flowchart TD
    A[wirestead::tcp_client host, port] --> B[TcpClientBuilder]
...
wirestead는 transport별로 서로 다른 entry point를 제공한다.
...
wirestead에서 wrapper는 transport별 구현을 감싸면서도 사용자에게 공통적인 조작 방식을 제공한다.
...
wirestead는 이 차이를 `ChannelInterface`와 `ServerInterface`로 분리한다.
...
따라서 wirestead는 공통화할 수 있는 영역은 공통화하되, 통신 모델 자체가 다른 부분은 interface를 분리한다.

## Framer와 Backpressure의 Public API 노출

wirestead의 public API는 단순히 연결과 송수신만 제공하지 않는다.
...
wirestead는 builder 단계에서 framer를 설정할 수 있게 한다.
...
따라서 wirestead는 backpressure strategy와 threshold를 builder 설정으로 노출한다.
...
이 접근은 wirestead의 API 설계에서 중요한 기준이다.
...
```mermaid
flowchart TD
    A[wirestead.hpp Facade] --> B[Builder Entry Point]
...
`wirestead.hpp`가 없더라도 내부 header를 직접 include해 객체를 만들 수는 있다.
...
- `wirestead::tcp_client(...)`로 TCP client를 만들기 시작한다.
...
wirestead의 방향은 “모든 것을 하나의 타입으로 통합”하는 것이 아니다.
...
`wirestead.hpp`가 여러 public wrapper와 builder를 모으는 역할을 하기 때문에, 작은 기능만 사용하는 경우에도 비교적 넓은 public surface를 포함하게 된다.
...
`wirestead::tcp_client(...)` 같은 entry point는 사용하기 쉽지만, 내부적으로 어떤 builder와 wrapper가 연결되는지 처음에는 감춰진다. 따라서 문서와 예제에서 이 흐름을 명확히 설명해야 한다.
...
wirestead의 Unified API는 다음 구조를 중심으로 구성된다.

```mermaid
mindmap
  root((wirestead Unified API))
    Facade
      wirestead.hpp
      public aliases
      builder entry points
...
wirestead에서 `wirestead.hpp`는 public entry point이고, Builder는 설정과 생성 흐름을 담당하며, Wrapper는 실제 runtime operation을 담당한다.
또한 Channel과 Server interface를 분리해 1:1 통신과 1:N 통신의 차이를 무리하게 숨기지 않는다.

정리하면 wirestead의 Unified API는 다음 목표를 가진다.
...
이러한 구조 때문에 wirestead는 여러 통신 방식을 지원하면서도 사용자는 비교적 일관된 방식으로 객체를 만들고 사용할 수 있다.
