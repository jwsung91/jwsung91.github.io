...
wirestead의 Memory / Buffer 설계는 이 문제를 다루기 위한 계층이다.
...
wirestead는 이 세 가지를 각각 다른 타입과 API로 표현한다.
...
wirestead의 buffer 설계에서 가장 중요한 원칙은 view와 ownership을 분리하는 것이다.
...
따라서 wirestead는 상황에 따라 다른 타입을 사용한다.
...
wirestead는 이 문제를 송신 API에서 명시적으로 나눈다.
...
wirestead의 MessageContext는 view와 copy API를 함께 제공한다.
...
wirestead에서 zero-copy는 “항상 복사가 없다”는 의미가 아니다.
...
wirestead에서 Memory / Buffer 설계는 비동기 통신의 lifetime과 ownership 문제를 다루기 위한 계층이다.
...
wirestead는 view, owning buffer, move ownership, shared ownership, memory pool을 구분함으로써 성능과 안전성 사이의 선택지를 API와 내부 구조에 반영한다.
