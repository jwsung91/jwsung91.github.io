...
- 문제: 기능 구현만으로는 다른 프로젝트에서 반복적으로 사용할 수 있는 라이브러리가 되지 않는다.
- 선택: 테스트, CMake 패키징, 문서, 릴리즈 흐름까지 포함해 사용 가능한 배포 단위로 정리한다.
- 포기한 것: 로컬에서만 동작하는 예제 수준 구현에 머물지 않고, 설치와 검증 비용을 설계 범위에 포함한다.
- 확인할 지표: 테스트 통과 범위, install/export 동작, 패키지 사용성, release checklist 완료 여부.
