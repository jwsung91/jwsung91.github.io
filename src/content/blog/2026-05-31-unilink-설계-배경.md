...
- 문제: transport마다 API와 lifecycle을 따로 만들면 애플리케이션 코드가 통신 방식 차이에 계속 묶인다.
- 선택: TCP, UDP, Serial, UDS를 하나의 개발 경험으로 묶는 공통 abstraction을 설계한다.
- 포기한 것: 각 transport의 세부 기능을 public API 전면에 모두 노출하는 방식을 선택하지 않는다.
- 확인할 지표: API 일관성, transport 추가 비용, callback 흐름, 사용자 코드의 조건 분기 수.
